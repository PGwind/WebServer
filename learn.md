...
11. 如果是文件，发送响应头 + 文件内容
12. 如果是目录，动态生成带 HTML 转义的目录页
13. 如果目标不存在或无权限，返回 `404` 或 `403`
14. 请求结束后输出统一访问日志，记录时间、客户端 IP、方法、路径、状态码
...
- 对不支持的方法返回 `405`
- 对非法请求行返回 `400`
- 对目录页中的文件名和目录名做 HTML 转义，避免页面破坏和基础 XSS
...
3. 为每个条目生成超链接、修改时间、大小信息，其中目录大小固定显示为 `-`
4. 拼成 HTML 表格并返回浏览器
...
- 在构建前检查 `pkg-config` 和 `libevent` 依赖
- 生成根目录可执行文件 `server`
...
仓库内置了 `tests/smoke_test.sh`，适合在目标 Linux 服务器上做最基础的回归验证。
...
这个脚本会临时创建测试目录、启动当前 `server` 二进制、用 `curl` 验证，然后自动清理测试资源。
...
- 根据权限失败返回 `403`
- 目录页输出增加 HTML 转义，降低文件名注入和页面破坏风险

### 9.3 工程化优化

- `start.sh` 支持端口和目录参数化
- `makefile` 支持 `pkg-config`，更适合 Ubuntu 部署
- `makefile` 在缺少 `pkg-config` 或 `libevent` 时会直接给出安装提示
- 去掉了写死的 404 绝对路径依赖
